...
7. [Phase 2.5: Grosir Configuration](#phase-25-grosir-configuration-admin)
8. [Phase 3: Group Buying Flow](#phase-3-group-buying-flow)
...
## Phase 2.5: Grosir Configuration (Admin)

**IMPORTANT:** These configurations must be set BEFORE creating a group buying session.

**Service:** Group Buying Service (http://localhost:3004/api-docs)

### 2.5.1 Set Bundle Configuration

**Endpoint:** `POST /api/admin/bundle-configs`

Sets units per wholesale bundle for each product/variant combination.

**For base product (no variant):**
```json
{
  "productId": "YOUR_PRODUCT_ID",
  "unitsPerBundle": 12
...
{
  "productId": "YOUR_PRODUCT_ID",
  "variantId": "YOUR_VARIANT_ID_SMALL",
  "unitsPerBundle": 2
}
```
...
{
  "productId": "YOUR_PRODUCT_ID",
  "variantId": "YOUR_VARIANT_ID_MEDIUM",
  "unitsPerBundle": 5
}
```
...
{
  "productId": "YOUR_PRODUCT_ID",
  "variantId": "YOUR_VARIANT_ID_LARGE",
  "unitsPerBundle": 4
}
```

**Note:** The `variantId` allows defining different bundle sizes per variant. A wholesale bundle of 12 units might contain: 2 Small + 5 Medium + 4 Large + 1 XL.

### 2.5.2 Set Warehouse Tolerance

**Endpoint:** `POST /api/admin/warehouse-tolerances`

Sets maximum excess units allowed and clearance rate estimates per product.
...
This defines how many extra units above MOQ the warehouse can accept for a product (to handle rounding up to full bundles).

### 2.5.3 Set Variant Allocations

**Endpoint:** `POST /api/admin/variant-allocations`

Sets maximum allocation quantity limits per variant for a session.
...
**Note:** This is typically set after creating the group buying session (in Phase 3.1), to limit how many units of each variant can be purchased in that session.

### 2.5.4 Verify Product Configuration

**Endpoint:** `GET /api/admin/products/{productId}/config`

Verify all grosir configuration is complete before creating a session.

Expected response includes:
- Bundle configurations
- Warehouse tolerances
- Product details with MOQ
...
- [ ] Publish product
- [ ] Set bundle configurations (per variant)
- [ ] Set warehouse tolerances
- [ ] Verify product configuration
- [ ] Create group buying session
- [ ] Set variant allocations for session
